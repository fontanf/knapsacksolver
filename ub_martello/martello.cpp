...
Profit ub_cardinality_max(const Instance& instance, ItemIdx k)
{
...
Profit ub_cardinality_min(const Instance& instance, ItemIdx k)
{
...
Profit ub_martello(const Instance& instance, Profit lower_bound)
{
	...
	if (kw <= b) {
		Profit ub = ub_cardinality_max(instance, kw);
	...
	if (kz >= b) {
		Profit ub = ub_cardinality_min(instance, kz);
		DBG(std::cout << "UB: " << ub << " (" << instance.optimum() << ")" << std::endl;)
		assert(instance.optimum() == 0 || ub >= instance.optimum());
		DBG(std::cout << "ub_martello()... end" << std::endl;)
		return ub;
	...
	Profit ub1 = ub_cardinality_max(instance, b);
	DBG(std::cout << "ub1: " << ub1 << std::endl;)
	Profit ub2 = ub_cardinality_min(instance, b);
