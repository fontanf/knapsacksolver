...
void branch_and_bound_rec(BranchAndBoundData& d)
{
    ...
    for (ItemPos j = d.j; j <= d.instance.last_item(); j++) {
        ...
        if (!d.sort) { // Bounding test
            if (upper_bound_0(d.instance, j, d.sol_curr, d.j_max) <= d.output.lower_bound)
                continue;
        } else {
            if (upper_bound_dembo(d.instance, j, d.sol_curr) <= d.output.lower_bound)
                return;
        }
        ...
        d.j = j + 1;
        branch_and_bound_rec(d);
        d.sol_curr.set(j, false);
    }
}

Output knapsacksolver::knapsack::branch_and_bound(
        Instance& instance,
        bool sort,
        Info info)
{
    info.os() << "*** branch_and_bound" << ((sort)? " (sort)": "") << " ***" << std::endl;
    ...
    Profit ub = (!sort)?
        upper_bound_0(instance, 0, 0, instance.capacity(), j_max):
