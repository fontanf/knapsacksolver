...
public:
    ...
    ItemPos before_break_item(DBG(Info& info)) const;
    ItemPos max_weight_item(DBG(Info& info)) const;
     ...
     * (Pisinger, 1997).
     */
    void sort_partially(DBG(Info& info COMMA) ItemIdx limit = 4);
    ...
    ItemPos bound_item_left(ItemPos s, Profit lb DBG(COMMA Info& info));
    ItemPos bound_item_right(ItemPos t, Profit lb DBG(COMMA Info& info));
    /**
     ...
     * 1999).
     */
    void init_combo_core(DBG(Info& info));

    /*
     * Move item j to the initial core and carefully update int_right_ and
     * int_left_.
     */
    void add_item_to_core(ItemPos s, ItemPos t, ItemPos j DBG(COMMA Info& info));
    ...
    void set_first_item(ItemPos k DBG(COMMA Info& info));
...
private:
    ...
    bool check();
    bool check_partialsort(DBG(Info& info)) const;
    ...
    void compute_break_item(DBG(Info& info));
    /** Remove items which weight is greater than the updated capacity */
    void remove_big_items(DBG(Info& info));

    void sort_right(Profit lb DBG(COMMA Info& info));
    void sort_left(Profit lb DBG(COMMA Info& info));
